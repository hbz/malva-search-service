...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="titleInfo">
        <xsl:apply-templates select="es:VolumeDesignation|es:TitleStatement[position()=2]|es:TitleStatement|es:TitleProper|es:SecondaryEdition|es:TitleUniform|es:TitleParallel|es:TitleParallelStatement"/>
    </xsl:template>
    ...
    <xsl:template match="es:TitleStatement">
        <mods:titleInfo>
            <mods:title>
                <xsl:value-of select="es:titleMain"/>
            </mods:title>
            <xsl:call-template name="partNumber"/>
            <xsl:call-template name="subTitle"/>
        </mods:titleInfo>
    </xsl:template>
    <xsl:template match="es:SecondaryEdition">
        <mods:titleInfo>
            <mods:title>
    ...
    <xsl:template name="name">
        <xsl:apply-templates select="es:Person|es:PersonCreator|es:PersonContributor|es:PersonInterpreter|es:PersonAddressee|es:Conference|es:Geo|es:CorporateBody"/>
    </xsl:template>
    ...
    <xsl:template name="personal">
        <xsl:param name="p"/>
        <mods:namePart>
            <xsl:value-of select="$p/es:personName"/>
        </mods:namePart>
        <xsl:if test="$p/es:personNumbering">
            <mods:namePart type="termsOfAddress">
                <xsl:value-of select="$p/es:personNumbering"/>
            </mods:namePart>
        </xsl:if>
        <xsl:if test="$p/es:personTitle">
            <mods:namePart type="termsOfAddress">
                <xsl:value-of select="$p/es:personTitle"/>
            </mods:namePart>
        </xsl:if>
        <xsl:if test="$p/es:personBio">
            <mods:namePart type="date">
                <xsl:value-of select="$p/es:personBio"/>
            </mods:namePart>
        </xsl:if>
        <xsl:if test="$p/es:personRole">
            <mods:role>
                <mods:roleTerm type="text">
                    <xsl:value-of select="$p/es:personRole"/>
                </mods:roleTerm>
            </mods:role>
        </xsl:if>
    ...
    <xsl:template match="es:CorporateBody|es:Geo">
        <mods:name type="corporate">
            <mods:namePart>
                <xsl:for-each select="es:corporateBodyName|es:corporateBodyOther|es:corporateBodyNameUnit|es:geoName|es:geoOtherInformation|es:geoSubUnit|es:geoZone">
                    <xsl:value-of select="."/>
    ...
    <xsl:template match="es:Conference">
        <mods:name type="conference">
            <mods:namePart>
                <xsl:for-each select="es:conferenceName|es:conferenceDate|es:conferencePlace">
                    <xsl:value-of select="."/>
                    <xsl:if test="position() != last()">
                        <xsl:text>, </xsl:text>
                    </xsl:if>
    ...
    <xsl:template name="typeOfResource">
        <xsl:variable name="format" select="es:dc/es:format"/>
        <xsl:choose>
            <xsl:when test="$format = 'Gedruckte Ressource'">
